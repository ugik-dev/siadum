...
class Printer
{
    ...
    public function generate_print($business_info, $data)
    {
        ...
        try {
            ...
            if ($business_info['customer']) {
                ...
                $printer->setTextSize(1, 1);
                $printer->text('Customer : ' . $business_info['customer'] . "\n" . 'Sales : ' . $business_info['served']);
                $printer->text("\n");
            }
            ...
            $printer->text($this->getPad("Diskon:", 'product'));
            $printer->text($this->getPad('', 'price'));
            $printer->text($this->getPad('', 'qty'));
            ...
            $printer->setEmphasis(true);
            $printer->text($this->getPad("Grand Total:", 'product'));
            $printer->text($this->getPad('', 'price'));
            $printer->text($this->getPad('', 'qty'));
            ...
            if ($business_info['customer_id'] != 2) {
                ...
                $printer->text($this->getPad("Dibayar:", 'product'));
                $printer->text($this->getPad("", 'price'));
                $printer->text($this->getPad("", 'qty'));
    ...
    */
                $printer->text($this->getPad("Sisa Hutang:", 'product'));
                $printer->text($this->getPad("", 'price'));
                $printer->text($this->getPad("", 'qty'));
            ...
            $printer->setTextSize(1, 1);
            $printer->setFont(0);
            $printer->text("PT. Indometal Asia\n");
            $printer->setJustification(P::JUSTIFY_CENTER);
